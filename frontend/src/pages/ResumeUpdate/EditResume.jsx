...
import ProfileInfoForm from '../../pages/ResumeUpdate/Forms/ProfileInfoForm.jsx'
import ContactInfoFrom from '../../pages/ResumeUpdate/Forms/ContactInfoFrom.jsx'
import WorkExperienceFrom from './Forms/WorkExperienceFrom.jsx';
import EducationDetailsForm from './Forms/EducationDetailsForm.jsx';
import SkillsInfoForm from './Forms/SkillsInfoForm.jsx';
import ProjectsDetailFrom from './Forms/ProjectsDetailFrom.jsx';
import CertificationInfoFrom from './Forms/CertificationInfoFrom.jsx';
import AdditionalInfoFrom from './Forms/AdditionalInfoFrom.jsx';
...
const EditResume = () => {
    ...
    switch(currentPage) {
      ...
      case "work-experience": 
        resumeData.workExperience.forEach( 
          ({company, role, startDate, endDate}, index) =>{
            if(!company.trim())
              errors.push(`Company is required in work experience ${index + 1}`);
            if(!role.trim())
        ...
        resumeData.skills.forEach(
          ({name, progress}, index) =>{
            ...
            if(progress < 0 || progress > 100)  
              errors.push(`Skill Progress must be between 0 and 100 for skill ${index + 1}`);
          }   
          );
         break;
      case "projects":
      ...
      case "additionalInfo":
         if(
          ...
          !resumeData.language[0].name?.trim()
         ) {
  ...
  const addArrayItem = (section, newItem) => {
    setResumeData((prev) => ({
      ...prev,
      [section]: [...(prev[section] || []), newItem], // Added || [] to ensure it's iterable
    }));
  };
...
const uploadResumeDetails = async (thumbnailLink, profilePreviewUrl) => {
  try {
    ...
    const payload = {
      ...
      profilePreviewUrl: profilePreviewUrl || null,
      template: typeof resumeData.template === 'string'
        ? resumeData.template
        : resumeData.template?.theme || 'default',
...
const fetchResumeDetailsById = async () => {
  try {
    ...
    if (response.data && response.data.profileInfo) {
      ...
      setResumeData((prevState) => ({
        ...
        colorPalette: resumeInfo?.colorPalette || ['blue'],
  ...
  return (
    <DashboardLayout>
      <div className='container mx-auto'>
        <div className="flex items-center justify-center gap-5 bg-white rounded-lg border border-purple-100 py-3 px-4 mb-4">
          <TitleInput
  ...
  <div className="px-5 pb-5">
    {errorMsg && (
      <div className="my-2 flex items-center gap-2 rounded bg-red-100 px-3 py-1 text-[11px] font-medium text-red-700">
    ...
    <div className="flex items-end justify-end gap-3 mt-3 mb-5">
      ...
      >
        {currentPage === "additionalInfo" ? (
          ...
          <>
            Next
              <LuArrowLeft className="rotate-180 text-[16px]" />
               </>
            )}
              </button>
             </div>
